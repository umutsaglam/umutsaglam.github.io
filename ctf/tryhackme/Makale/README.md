...
- Modelleme ortamını, yani bir saldırganın beyaz şapka saldırısı kullanarak sisteme nasıl saldırabileceğini tanımlar.
- Hacker’ın (saldırganın) bilgisayar işlevlerine ve verilerine erişmek için saldırabileceği zayıf noktaların bulunmasına yardımcı olur. 
- Orijinal verileri korunarak siyah şapka saldırılarını önlemek için destek noktaları oluşturulur.
- Potansiyel bir işletmeye yapılan saldırının büyüklüğü ve etkisi hakkında tahmin yürütülür.
- Bilişim teknolojilerinin güvenliğine yatırımı artırmanın neden önemli olduğunu gösteren kanıtlar sağlar.


Sızma / Penetrasyon testi belirli aşamalardan oluşur :

1. Bilgi Toplama: Hedef sistem hakkında ulaşılabilen veriler toplanır
2. Ağ Haritasının Çıkarılması: Edinilen bilgiler sonucu hedefin ağ haritası çıkarılır
3. Zafiyet Taraması: Çeşitli araçlar yardımı ile ya da manuel olarak sistemde zafiyet aranır
4. Sisteme Sızma (Zafiyet İstismarı): Zafiyet bulunduğu takdirde bu zafiyetin istismar edilmesini içerir
5. Yetki Yükseltme: Sistemde yönetici yetkisine (admin, root…) sahip olup sistem kontrol yeteneği kazanmayı hedefler

Online labaratuarlarda sanal makineler ile sızma testi yapılır buna bayrak yakalama (CTF) yarışmaları denir.
Bu makalede sanal ortamda yapılan sızma testi anlatılacaktır.


## Giriş

İşletim sistemi olarak Debian Linux dağıtımından türetilen Kali Linux'ü kullancağız.Kali Linux, temel olarak gelişmiş sızma testleri ve güvenlik için kullanılır. Bilgisayar korsanlarına ek olarak sızma testçileri, güvenlik araştırmacıları, adli tıp görevlileri ve tersine mühendislik yapmak isteyenler tarafından tercih edilir. Çünkü bu işletim sistemi bilgi güvenliği görevlerine yönelik yüzlerce araca sahiptir. Bu testte işletim sistemi içerinde olan nmap, gobuster, sqlmap araçları kullanılacaktır.


TryHackMe sitesinde bulunan CyberCrafted makinesini çözeceğiz. Bize makinenin IP adresi veriliyor ve bu makineye erişim elde etmemiz isteniyor.

Öncelikle bilgi toplama aşamasından başlıyoruz. Bunun için nmap aracını kullanıyoruz. Nmap ismini Network Mapper'in kısaltmasından alır. Sistemde hangi cihazların çalıştığını belirlemek, mevcut ana makineleri ve sundukları hizmetleri keşfetmek, açık bağlantı noktaları bulmak ve güvenlik risklerini taramak için kullanılır.
...
Makineyi taradığımızda ssh http ve minecraft servislerinin çalıştığını görüyoruz.

Herhangi bir alt domain (subdomain) olup olmadığını görmek için ffuf aracını kullanıyoruz.
...
Şimdi herhangi bir alt alan adına web tarayıcısından erişmeye çalıştığımızda sayfalar yüklenmiyor, aşağıdaki görüntüye benzer bir çıktı alıyoruz.

![](https://github.com/umutsaglam/CTF-Writeups/blob/main/TryHackMe/CyberCrafted/images/a1.png?raw=true)

/etc/hosts dosyasına ip adresi ve domain, subdomain adreslerini ekleyiyoruz. Bu şekilde sorunumuz çözülüyor.
...
Sayfalara göz gezdiğimde herhangi bir güvenlik açığı görmüyoruz.

Gobuster aracını kullanarak gizli dizinleri araştırıyoruz.
...
Verilen hash'i [crackstation](crackstation.net) sitesinde kırıyoruz.
...
Kullanıcı adı ve şifreyi öğrendiyoruz ve login sayfasından giriş yapıyoruz.
...
Bize verilen komut çalıştırma sayfasına PHP Reverse Shell kodu yazarak makinenin bize bağlanmasını sağlıyoruz.
