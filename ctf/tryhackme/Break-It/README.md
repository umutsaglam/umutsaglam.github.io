# Break It

## Giriş
Bir başka ctf yazıma hoşgeldiniz! Bu TryHackMe  <a href="https://tryhackme.com/room/breakit">Break It</a> odasında öğrenilmeye değer tonlarca şey var. Lafı daha fazla uzatmadan konuya girelim.

## Background

> Kodları kırabilir misin?

> Zorluk: Orta 

```
Bu oda, kod çözme/kodlama konusunda temel bilgi gerektirir.
```
# Bölüm 1 - Bases

1.

```
[Super easy] MVQXG6K7MJQXGZJTGI======
```
**Base32** kodlu bir dizedir. Bunu çözmek için `base32`yi kullanabiliriz.

```
┌──(root@r3tr0)-[~/ctf/thm/ctf/Break_it]
└─# echo "MVQXG6K7MJQXGZJTGI======" | base32 -d
easy_base32
```

2. 

```
[Easy] TVJYWEtZVE1NVlBXRVlMVE1WWlE9PT09
```

**Base64** kodlu bir dizedir. Bunu çözmek için `base64`ü kullanabiliriz.

```
┌──(root@r3tr0)-[~/ctf/thm/ctf/Break_it]
└─# echo "TVJYWEtZVE1NVlBXRVlMVE1WWlE9PT09" | base64 -d 
MRXXKYTMMVPWEYLTMVZQ====
```
Double encoded? 

```
┌──(root@r3tr0)-[~/ctf/thm/ctf/Break_it]
└─# echo "TVJYWEtZVE1NVlBXRVlMVE1WWlE9PT09" | base64 -d | base32 -d
double_bases
``` 

3. 

```
[Moderate] GM4HOU3VHBAW6OKNJJFW6SS2IZ3VAMTYORFDMUC2G44EQULIJI3WIVRUMNCWI6KGK5XEKZDTN5YU2RT2MR3E45KKI5TXSOJTKZJTC4KRKFDWKZTZOF3TORJTGZTXGNKCOE======
```

Bu **`base32`** tipi bir kodlama.

```
┌──(root@r3tr0)-[~/ctf/thm/ctf/Break_it]
└─# echo "GM4HOU3VHBAW6OKNJJFW6SS2IZ3VAMTYORFDMUC2G44EQULIJI3WIVRUMNCWI6KGK5XEKZDTN5YU2RT2MR3E45KKI5TXSOJTKZJTC4KRKFDWKZTZOF3TORJTGZTXGNKCOE======" | base32 -d            
38wSu8Ao9MJKoJZFwP2xtJ6PZ78HQhJ7dV4cEdyFWnEdsoqMFzdvNuJGgy93VS1qQQGefyqw7E36gs5Bq
```

Bir **`base58`** dizisi.

```
┌──(root@r3tr0)-[~/ctf/thm/ctf/Break_it]
└─# echo "GM4HOU3VHBAW6OKNJJFW6SS2IZ3VAMTYORFDMUC2G44EQULIJI3WIVRUMNCWI6KGK5XEKZDTN5YU2RT2MR3E45KKI5TXSOJTKZJTC4KRKFDWKZTZOF3TORJTGZTXGNKCOE======" | base32 -d | base58 -d
59 6d 46 7a 5a 54 45 32 58 32 6c 7a 58 32 68 6c 65 41 3d 3d    
```

Bu sefer **hexadecimal** dizisi elde ediyoruz. Diziyi çevirmek için `xxd` kullanabiliriz:

```
┌──(root@r3tr0)-[~/ctf/thm/ctf/Break_it]
└─# echo "GM4HOU3VHBAW6OKNJJFW6SS2IZ3VAMTYORFDMUC2G44EQULIJI3WIVRUMNCWI6KGK5XEKZDTN5YU2RT2MR3E45KKI5TXSOJTKZJTC4KRKFDWKZTZOF3TORJTGZTXGNKCOE======" | base32 -d | base58 -d | xxd -r -p
YmFzZTE2X2lzX2hleA==
```

yine **`base64`**.

```
┌──(root@r3tr0)-[~/ctf/thm/ctf/Break_it]
└─# echo "GM4HOU3VHBAW6OKNJJFW6SS2IZ3VAMTYORFDMUC2G44EQULIJI3WIVRUMNCWI6KGK5XEKZDTN5YU2RT2MR3E45KKI5TXSOJTKZJTC4KRKFDWKZTZOF3TORJTGZTXGNKCOE======" | base32 -d | base58 -d | xxd -r -p | base64 -d   
base16_is_hex
```

Başardık!

4. 

```
[Hard] HRBUGQDUHFWDIXKUIBWXIJTHIE3DCY3BIE2FKQSZHNDE6MRUIA2TWWDMHRBV2ZKCHQWFCTLPIE2EEJDBIBZCEW3OHUSTOLRCHNFGMVC6IFJXIQ2AHVMVONSBHVOVIM2MHVPV42J4HQVCQ4REIFHVIJ2WHFWDYQSUHROGILJCIFIU23CXHNCEIXK2HRDVSXKOHV2SQJLC
```

Bize kolaylık sağlaması için, [CyberChef](https://gchq.github.io/CyberChef) sitesini kullanacağız:

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a1.png)

- Çıktı: `that_is_a_lot_of_bases`

5. 

```
[Insane] 


```

**Aynı şekilde CyberChef'e kopyala yapıştır yapıyoruz:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a2.png)

Manuel olarak yapmayı deneyelim:

Öncelikle bu bir **`base32`** dizesi.

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a3.png)

**`base85`** gibi görünüyor, çünkü daha fazla özel karaktere sahip.

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a4.png)

Bu sefer **`base10`** dizesi:

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a5.png)

Bu bir **hexadecimal** dizesi, veya `base16`:

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a6.png)

Belki de **`base91`**? Bunu çözmek için [dcode.fr](https://www.dcode.fr/base-91-encoding) sitesini kullanacağım:

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a7.png)

 **`base58`** gibi görünüyor:

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a8.png)

**hexadecimal:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a9.png)

**base64:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a10.png)

**base10:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a11.png)

**hexadecimal:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a12.png)

Sonunda bulduk!

# Görev 2 - Base and cipher

> Peki ua base ve cipherı aynı anda kullanırsak?

1. 

```
[Easy] PJXHQ4S7GEZV6ZTDOZQQ====
```

A **`base32`** .

```
┌──(root@r3tr0)-[~/ctf/thm/ctf/Break_it]
└─# echo "PJXHQ4S7GEZV6ZTDOZQQ====" | base32 -d
znxr_13_fcva
```

**rot13??**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a13.png)

2. 

```
[Moderate] NjZMKVhATl1EcEI2Jio4Q0xuVy1EZSo5ZkFLV0M6QVUtPFpGQ0InIkReYg==
```

**`base64`** .

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a14.png)

**base85:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a15.png)

**vigenre cipher**?

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a16.png)

3. 

```
[Hard] -!r/X,]n/Z-Zs\X,X$,rI<@]#-9Oh,-=A]R-p9\+
```

**`base85`**:

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a17.png)

**`base91`**?

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a18.png)

**base64:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a19.png)

**base32:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a20.png)

Görünüşe göre **döndürülmüş**:

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a21.png)

4. 

```
[Insane]

```

**`base32`**:

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a22.png)

**base85:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a23.png)

**base91:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a24.png)

**base10:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a25.png)

**hexadecimal:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a26.png)

**base64:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a27.png)

vigenere cipher (key = **tangodown**):

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a28.png)

Fakat **bu düzgün bir dize değil**. Geri dönelim.

**base58:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a29.png)

**vigenere decode**:

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a30.png)

Başardık!!

# Görev 3 - Base, cipher, bit shift

> Bu kombinasyonu çözebilir misin? Belki Hex workshop yardımcı olabilir.

**[Hex workshop](http://www.hexworkshop.com/)!**

1.

```
[Moderate] 2D 37 2B 19 31 99 31 B3 B2 AB A5 18 32 37 20 B3 B2 AC 2D 1A 31 B4 A1 3A A4 A3 9C B4 AD 36 AC 9E
```

- Hex Workshop'u açın, yeni bir dosya oluşturun ve "File" -> "New":

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a31.png)

- Click "Paste Special" -> check "Interpet as a hexadecimal string" -> choose "CF_TEXT" -> click "paste":

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a32.png)

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a33.png)

**Block shift left:**

- Select "Block Shift Left" via "Tools" -> "Operations" -> "Block Shift Left":

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a34.png)

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a35.png)

**base64:**

```
┌──(root@r3tr0)-[~/ctf/thm/ctf/Break_it]
└─# echo "ZnV2c2cgeWJ0dnAgeXZ4ciBuIG9iZmY=" | base64 -d                
fuvsg ybtvp yvxr n obff
```

**rot13:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a36.png)

2. 

```
[Hard] 19 1A 1C 99 1C 9C A8 38 27 B2 34 27 B9 27 26 28 3C 3B AA 28 A5 AA 2A 29 3C 29 B2 B2 21 B1 AC A6 1C AC 29 A8 38 99 2C
```

- Kopyalayın ve yapıştırın:

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a37.png)

**Shift left:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a38.png)

- "8 Bit Unsigned Byte" seçeneğini seçin:

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a39.png)

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a40.png)

**base58:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a41.png)

**rot13:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a42.png)

3. 

```
[Insane] A9 A8 2B EE 2A AA C9 C8 C9 AA A8 0F 2A 8A AA EE A9 8A CA A8 A9 4F 6D 46 E9 A8 A8 0F A9 8A 6D 86 A9 AA A9 26 2A 8A 48 E8
```

- Kopyalayın ve yapıştırın:

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a43.png)

**Rotate left:**

- "Rotate Left" "Tools" -> "Operations" -> "Rotate Left":

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a44.png)

- "8 Bit Unsigned Byte":

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a45.png)

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a46.png)

**Rotate left:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a45.png)

**Rotate left:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a45.png)

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a47.png)

**base64:**
```
┌──(root@r3tr0)-[~/ctf/thm/ctf/Break_it]
└─# echo "MEYwQUNFNUExQTUwMTVEMzk2OEExMTk4MUM1QTBG" | base64 -d        
0F0ACE5A1A5015D3968A11981C5A0F
```

- Kopyalayın ve yapıştırın:

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a48.png)

**Block shift left twice:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a49.png)

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a50.png)

**base85:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a51.png)

**rot13:**

![](https://raw.githubusercontent.com/siunam321/CTF-Writeups/main/TryHackMe/Break-It/images/a52.png)

Sonunda başardık!

# Sonuç

Neler Öğrendik:

1. Base Encoding & Decoding
2. Cipher
3. Bit Shift
